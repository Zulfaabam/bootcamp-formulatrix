...
for (let i = 0; i < slideCount; i++) {
  ...
  await page.evaluate((index) => {
    ...
    // If there's a SlidePresentation class instance, use its native method and skip manual overrides
    if (window.presentation) {
      if (typeof window.presentation.showSlide === 'function') {
        window.presentation.showSlide(index);
        return;
      }
      if (typeof window.presentation.goToSlide === 'function') {
        window.presentation.goToSlide(index);
        return;
      }
    }

    // Fallback Strategy: Direct slide manipulation
